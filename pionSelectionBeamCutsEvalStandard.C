//I modified Richie`s code for pion selection
#include "TH1.h"
#include "TGraph.h"
#include "TH2.h"
#include "TF1.h"
...
void pionSelectionBeamCutsEval( int selection_ID, bool data, int mom=6)
{
...
std::string inelProcess="pion+Inelastic";

std::string fileMC="/exp/dune/data/users/nbostan/...root";
std::string fileData="/exp/dune/data/users/nbostan/...root";
if (mom==7)
{
fileMC="/exp/dune/data/users/nbostan/...root";
fileData="/exp/dune/data/users/nbostan/...root";

}
TFile f(Form("%s",fileData.c_str()));
TFile fMC(Form("%s",fileMC.c_str()));
...
TH1D *h1d_reco_sel_endZ_K=new TH1D("reco_endZ_K","reco_endZ_K",nBin,edges);
...
TH1D *h1d_reco_sel_cheat_other_endZ=new TH1D("reco_cheat_pi_other_endZ","reco_cheat_pi_other_endZ",nBin,edges);
...
if(selection_ID==5){
tMC->Project("reco_cheat_endZ",Form("%s",var.c_str()),Form("true_beam_endProcess==\"%s\" && (true_beam_endZ>30 && true_beam_endZ<220.0) && reco_beam_true_byE_ID==true_beam_ID && selection_ID<%d",inelProcess.c_str(), selection_ID));
...
tMC->Project("reco_cheat_outOfVol_endZ",Form("%s",var.c_str()),Form(" (true_beam_endZ<30 || true_beam_endZ>220.0)  && reco_beam_true_byE_ID==true_beam_ID  && selection_ID<%d", selection_ID));
tMC->Project("reco_cheat_pi_other_endZ",Form("%s",var.c_str()),Form(" (true_beam_endZ>30 && true_beam_endZ<220.0) && true_beam_endProcess!=\"%s\" && reco_beam_true_byE_ID==true_beam_ID  && selection_ID<%d",inelProcess.c_str(), selection_ID));

tMC->Project("reco_cheat_sec_endZ",Form("%s",var.c_str()),Form("(reco_beam_true_byE_PDG)==211 && reco_beam_true_byE_ID!=true_beam_ID && selection_ID<%d ", selection_ID));
...
tMC->Project("reco_endZ_K",Form("%s",var.c_str()),Form("abs(reco_beam_true_byE_PDG)==321 && selection_ID<%d  ", selection_ID));
...
tMC->Project("reco_endZ_other",Form("%s",var.c_str()),Form("(reco_beam_true_byE_PDG>3000 || reco_beam_true_byE_PDG==-211)  && selection_ID<%d ", selection_ID));
...
tMC->Project("reco_cheat_outOfVol_endZ",Form("%s",var.c_str()),Form(" (true_beam_endZ<30 || true_beam_endZ>220.0) && reco_beam_true_byE_ID==true_beam_ID  && selection_ID<=%d", selection_ID));
tMC->Project("reco_cheat_k_other_endZ",Form("%s",var.c_str()),Form(" (true_beam_endZ>30 && true_beam_endZ<220.0) && true_beam_endProcess!=\"%s\" && reco_beam_true_byE_ID==true_beam_ID  && selection_ID<=%d",inelProcess.c_str(), selection_ID));
tMC->Project("reco_cheat_sec_endZ",Form("%s",var.c_str()),Form("(reco_beam_true_byE_PDG)==211 && reco_beam_true_byE_ID!=true_beam_ID && selection_ID<=%d ", selection_ID));
tMC->Project("reco_endZ_p",Form("%s",var.c_str()),Form("abs(reco_beam_true_byE_PDG)==2212 && selection_ID<=%d  ", selection_ID));
tMC->Project("reco_endZ_mu",Form("%s",var.c_str()),Form("abs(reco_beam_true_byE_PDG)==13 && selection_ID<=%d", selection_ID));
tMC->Project("reco_endZ_K",Form("%s",var.c_str()),Form("abs(reco_beam_true_byE_PDG)==321 && selection_ID<=%d  ", selection_ID));
tMC->Project("reco_endZ_e",Form("%s",var.c_str()),Form("(abs(reco_beam_true_byE_PDG)==11 || reco_beam_true_byE_PDG==22) && selection_ID<=%d   ", selection_ID));
tMC->Project("reco_endZ_other",Form("%s",var.c_str()),Form("(reco_beam_true_byE_PDG>3000 || reco_beam_true_byE_PDG==-211 || (reco_beam_true_byE_PDG)==-999)  && selection_ID<=%d ", selection_ID));
tMC->Project("reco_endZ_noreco",Form("%s",var.c_str()),Form("selection_ID==%d ", selection_ID));
tMC->Project("reco_endZ",Form("%s",var.c_str()),Form("selection_ID<=%d",selection_ID));
}
...
h1d_reco_sel_endZ_K->SetFillColor(kGray);
...
h1d_reco_sel_endZ_K->SetLineColor(kGray);
...
h1d_reco_sel_cheat_endZ->SetLineWidth(0);//kGreen+3);
h1d_reco_sel_cheat_outOfVol_endZ->SetLineWidth(0);//kViolet-2);
h1d_reco_sel_endZ_p->SetLineWidth(0);//kRed-10);
h1d_reco_sel_endZ_mu->SetLineWidth(0);//kYellow);
h1d_reco_sel_endZ_K->SetLineWidth(0);//kGray);
h1d_reco_sel_endZ_e->SetLineWidth(0);//kOrange);
h1d_reco_sel_endZ_other->SetLineWidth(0);//kBlue-10);
h1d_reco_sel_cheat_other_endZ->SetLineWidth(0);//kGreen);
h1d_reco_sel_cheat_sec_endZ->SetLineWidth(0);//kCyan);
h1d_reco_sel_endZ_noreco->SetLineWidth(0);//0);
...
h1d_reco_stack_endZ->Add(h1d_reco_sel_endZ_K);
...
std::cout<<h1d_reco_sel_endZ->GetEntries()<<','<<h1d_reco_sel_cheat_endZ->GetEntries()<<','<<h1d_reco_sel_cheat_other_endZ->GetEntries()<<','<<h1d_reco_sel_cheat_sec_endZ->GetEntries()<<','<<h1d_reco_sel_endZ_p->GetEntries()<<','<<h1d_reco_sel_endZ_K->GetEntries()<<','<<h1d_reco_sel_endZ_mu->GetEntries()<<','<<h1d_reco_sel_endZ_e->GetEntries()<<','<<h1d_reco_sel_endZ_other->GetEntries()<<std::endl;
...
h1d_reco_stack_endZ->SetTitle(Form("%d GeV/c Sample: No Selection Cuts",mom)); 
  ...
  if (selection_ID==2) h1d_reco_stack_endZ->SetTitle(Form("%d GeV/c Sample: #pi^{+} Inc.+Int. Cand.",mom));
  if (selection_ID==1) h1d_reco_stack_endZ->SetTitle(Form("%d GeV/c Sample: #pi^{+} Int. Cand.",mom));
  double norm=1.f;
if (!data && var.find("reco_beam_calibrated_interactingEnergy-true_beam_traj_interactingEnergy")==std::string::npos) {
  ...
  l->AddEntry(h1d_reco_sel_cheat_endZ,Form("#pi^{+} inel.: %3.1f",h1d_reco_sel_cheat_endZ->GetEntries()*norm),"f");
  l->AddEntry(h1d_reco_sel_cheat_outOfVol_endZ,Form("#pi^{+} inel. out of Fid. Vol.: %3.1f",h1d_reco_sel_cheat_outOfVol_endZ->Integral()*norm),"f");
 // l->AddEntry(h1d_reco_sel_cheat_endZ_broken,Form("kaonBrokenTrk: %3.0f",h1d_reco_sel_cheat_endZ_broken->GetEntries()),"f");
  l->AddEntry(h1d_reco_sel_cheat_other_endZ,Form("#pi^{+} decay: %3.1f",h1d_reco_sel_cheat_other_endZ->GetEntries()*norm),"f");
  l->AddEntry(h1d_reco_sel_cheat_sec_endZ,Form("Sec. Beam #pi^{+}: %3.1f",h1d_reco_sel_cheat_sec_endZ->GetEntries()*norm),"f");
  l->AddEntry(h1d_reco_sel_endZ_p,Form("Sec. p^{+/-}: %3.1f",h1d_reco_sel_endZ_p->GetEntries()*norm),"f");
  l->AddEntry(h1d_reco_sel_endZ_K,Form("Sec. K^{+/-}: %3.1f",h1d_reco_sel_endZ_K->GetEntries()*norm),"f");
...
l->AddEntry(h1d_reco_sel_endZ,Form("Total Sim. Evt.: %3.0f",h1d_reco_sel_endZ->GetEntries()),"lp");
  l->AddEntry(h1d_reco_sel_cheat_endZ,Form("#pi^{+} inel.: %3.1f",h1d_reco_sel_cheat_endZ->GetEntries()*norm),"f");
  l->AddEntry(h1d_reco_sel_cheat_outOfVol_endZ,Form("#pi^{+} inel. out of Fid. Vol.: %3.1f",h1d_reco_sel_cheat_outOfVol_endZ->GetEntries()*norm),"f");
 // l->AddEntry(h1d_reco_sel_cheat_endZ_broken,Form("kaonBrokenTrk: %3.0f",h1d_reco_sel_cheat_endZ_broken->GetEntries()),"f");
  l->AddEntry(h1d_reco_sel_cheat_other_endZ,Form("#pi^{+} decay: %3.1f",h1d_reco_sel_cheat_other_endZ->GetEntries()*norm),"f");
  l->AddEntry(h1d_reco_sel_cheat_sec_endZ,Form("Sec. Beam #pi^{+}: %3.1f",h1d_reco_sel_cheat_sec_endZ->GetEntries()*norm),"f");
  l->AddEntry(h1d_reco_sel_endZ_p,Form("Sec. p^{+/-}: %3.1f",h1d_reco_sel_endZ_p->GetEntries()*norm),"f");
...
if (var.find("true_beam_endZ")!=std::string::npos){
h1d_reco_stack_endZ->GetYaxis()->SetTitle("Number of Events");
h1d_reco_stack_endZ->GetXaxis()->SetTitle("True #pi^{+} Z Endpoint [cm]");
h1d_reco_stack_endZ->GetYaxis()->CenterTitle();
h1d_reco_stack_endZ->GetXaxis()->CenterTitle();
}
if(var.find("true_beam_endZ-reco_beam_calo_endZ")!=std::string::npos){
...
h1d_reco_stack_endZ->GetXaxis()->CenterTitle();
 ...
 h1d_reco_stack_endZ->GetXaxis()->SetTitleOffset(1.2);
    TLatex tL;
    tL.SetNDC();
    tL.DrawLatex(0.20,0.94,"#bf{DUNE:ProtoDUNE-HD}");
c1.Modified();
std::cout<<f1->GetParameter(0)<<','<<f1->GetParameter(1)<<','<<f1->GetParameter(2)<<std::endl;
c1.Print(Form("validationPlots/pionEval%s_%d_%dStandard%dGeV.png",var.c_str(),selection_ID,data,mom));
